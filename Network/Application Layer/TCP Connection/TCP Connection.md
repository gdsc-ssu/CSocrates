...
각 TCP Segment는 **Sequence** 번호와 **Checksum**을 갖는다.
...
만약 같이 보낼 패킷이 없다면, 0.1~0.2초 정도를 버퍼에 저장해두고 같이 보낼 수 있는 패킷을 찾은 후에 같이 보낼 수 있는 패킷이 없다면 별도의 패킷을 만들어 전송, 아니라면 같이 묶어서 보낸다.
...
#### 파이프라이닝 커넥션
HTTP/1.1 은 지속 커넥션을 통해 요청을 파이프라이닝, 즉 병렬 처리 할 수 있다. 
